	const unsigned int	fatOffset				= 25;			// FAT Offset. In sectors. Смещение 80, Размер 4.

	const unsigned char	bytePerSector2Power		= 9;							// Byte Per Sector Shift. Power of 2. Minimum 9 (512 bytes per sector), maximum 12 (4096 bytes per sector).
	const unsigned int	bytePerSector			= 512; 			// (2^bytePerSector2Power);

	const unsigned int	clusterHeapOffset		= 30;			// 256 Cluster Heap Offset. In sectors. Смещение 88, Размер 4.
	const unsigned int	clusterCount			= 256;			// Cluster Count. 2^32-11 is the maximum number of clusters could be described. Смещение 92, Размер 4.
	
	const unsigned int	firstClusterOfRootDir	= 5;			// Cluster Count. 2^32-11 is the maximum number of clusters could be described. Смещение 92, Размер 4.

	const unsigned int	volumeSerialNumber		= 0x8634EA57;	// Cluster Count. 2^32-11 is the maximum number of clusters could be described. Смещение 92, Размер 4.

	const unsigned short fileSystemRevision		= 1;

	const unsigned char sectorsPerCluster2Power	= 6;
	const unsigned int  sectorsPerCluster		= 64;			// (2 ^ sectorsPerCluster2Power)
	
	const unsigned char numberOfFats			= 1;

	const unsigned int	volumeLengthH = 0x00000000;				// Volume Length. Size of exFAT volume in sectors. Смещение 72, Размер 8.
	const unsigned int	volumeLengthL = (sectorsPerCluster * clusterCount) + clusterHeapOffset;					// 0x00030800 = 198656 sectors

	const unsigned int	fatLendth				= (clusterCount * 4) / bytePerSector + 1;			// FAT Lendth. In sectors. May exceed the required space in order to align the second FAT. Смещение 84, Размер 4.


	const unsigned int	volumeSize				= bytePerSector * (sectorsPerCluster * clusterCount + clusterHeapOffset);


	const char exFatVolumeLabel[]	= "MetkaMetka";





// Таблица bitmap индексы таблицы [первый кластер байта bitmap table][последний кластер байта bitmap table]
// Пример: bitMapMaskTable[1][4] = 1E - в байте bitmap table кластеры заполнены начиная с 1 по 4 => 0001 1110 = 1E
const unsigned char bitMapMaskTable[8][8] = {
...
const unsigned char exFatBootSectorBootCode[390] = {
	0x33, 0xC9, 0x8E, 0xD1, 0xBC, 0xF0, 0x7B, 0x8E, 0xD9, 0xA0, 0xFB, 0x7D, 0xB4, 0x7D, 0x8B, 0xF0,
	0xAC, 0x98, 0x40, 0x74, 0x0C, 0x48, 0x74, 0x0E, 0xB4, 0x0E, 0xBB, 0x07, 0x00, 0xCD, 0x10, 0xEB,
	0xEF, 0xA0, 0xFD, 0x7D, 0xEB, 0xE6, 0xCD, 0x16, 0xCD, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	,
...
const unsigned int	upcaseTableSize = 	5836;
const unsigned char	upcaseTable[] = {
